...
def TakeImages():
    ...
    if is_number(Id) and name.isalpha():
        ...
        with open(r'UserDetails\UserDetails.csv', 'r+') as csvFile:
            reader = csv.reader(csvFile, delimiter=',',)
            for row in reader:
                if len(row) and row[0] == Id.strip():
                    return
...
def TrackImages():
    ...
    df = pd.read_csv("UserDetails/UserDetails.csv", header=None)
    df.columns = ['Id', 'Name']
    cam = cv2.VideoCapture(0)
    ...
    while True:
        ...
        cv2.imshow('im', im)
        # Press q or X button to break loop
        if cv2.waitKey(1) == ord('q') or cv2.getWindowProperty('im', cv2.WND_PROP_VISIBLE) < 1:
            break
    cam.release()
    cv2.destroyAllWindows()
